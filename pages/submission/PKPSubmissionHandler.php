...
abstract class PKPSubmissionHandler extends Handler
{
    ...
    protected function showWizard(array $args, Request $request, Submission $submission): void
    {
        ...
        $templateMgr->setState([
            ...
            'i18nConfirmSubmit' => __('submission.wizard.confirmSubmit', ['context' => $context->getLocalizedName()]),
    ...
    protected function getSubmitUserGroups(Context $context, User $user): array
    {
        ...
        foreach ($userGroups as $userGroup) {
            if (in_array($userGroup->getRoleId(), [Role::ROLE_ID_MANAGER, Role::ROLE_ID_SITE_ADMIN, Role::ROLE_ID_AUTHOR])) {
                $canSubmit[] = $userGroup;
            }
        }

        // Users without a submitting role can submit as the default
        // author role if it allows self registration
        if (!count($canSubmit)) {
            $defaultAuthorUserGroup = Repo::userGroup()
                ->getCollector()
                ->filterByContextIds([$context->getId()])
                ...
                ->getMany()
                ->first();
            if ($defaultAuthorUserGroup && $defaultAuthorUserGroup->getPermitSelfRegistration()) {
                $canSubmit[] = $defaultAuthorUserGroup;
            }
        }
    ...
    protected function getDetailsStep(Request $request, Submission $submission, Publication $publication, array $locales, string $publicationApiUrl, array $sections): array
    {
        ...
        $sections = [
            [
                ...
                'form' => $titleAbstractForm->getConfig(),
            ],
        ];
    ...
    protected function getEditorsStep(Request $request, Submission $submission, Publication $publication, array $locales, string $publicationApiUrl): array
    {
        ...
        $sections = [
            [
                ...
                'form' => $hasMetadataForm ? $metadataForm->getConfig() : $commentsForm->getConfig(),
            ],
        ];
