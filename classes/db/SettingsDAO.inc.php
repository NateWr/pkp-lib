...
abstract class SettingsDAO extends DAO
{
    ...
    public function loadSettings($id)
    {
        ...
        foreach ($result as $row) {
            ...
            if ($row->locale == '') {
                ...
                $settings[$row->setting_name][$row->locale] = $value;
            }
        }
     ...
     */
    public function &getSetting($id, $name, $locale = null)
    {
     ...
     */
    public function installSettings($id, $filename, $paramArray = [])
    {
     ...
     */
    public function updateSetting($id, $name, $value, $type = null, $isLocalized = false)
    {
        $keyFields = ['setting_name', 'locale', $this->_getPrimaryKeyColumn()];
        if (!$isLocalized) {
            ...
            if (is_array($value)) {
                foreach ($value as $locale => $localeValue) {
                    $this->update('DELETE FROM ' . $this->_getTableName() . ' WHERE ' . $this->_getPrimaryKeyColumn() . ' = ? AND setting_name = ? AND locale = ?', [$id, $name, $locale]);
     ...
     */
    public function deleteSetting($id, $name, $locale = null)
    {
        $cache = $this->_getCache($id);
        if ($cache) {
