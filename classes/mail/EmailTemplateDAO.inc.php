...
class EmailTemplateDAO extends SchemaDAO
{
    ...
    public $schemaName = SCHEMA_EMAIL_TEMPLATE;

    /** @copydoc SchemaDAO::$tableName */
    public $tableName = 'email_templates';

    /** @copydoc SchemaDAO::$settingsTableName */
    public $settingsTableName = 'email_templates_settings';
    ...
    {
        $this->update(
            'DELETE FROM email_templates_settings WHERE locale = ?',
            [$locale]
        );
    }
    ...
    {
        $this->update(
            'DELETE FROM email_templates_default_data WHERE locale = ?',
            [$locale]
        );
    }
     ...
     * @return boolean
     */
    public function defaultTemplateIsInstalled($key)
    {
        $result = $this->retrieve(
     ...
     */
    public function installEmailTemplates($templatesFile, $locales = [], $returnSql = false, $emailKey = null, $skipExisting = false)
    {
        $xmlDao = new XMLDAO();
        $sql = [];
        ...
        foreach ($data['email'] as $entry) {
            ...
            if ($returnSql) {
                $sql = array_merge($sql, $additionalQueries);
            }
        }
        if ($returnSql) {
            return $sql;
     ...
     * @return array|boolean
     */
    public function installEmailTemplateLocaleData($templatesFile, $locales = [], $returnSql = false, $emailKey = null)
    {
        $xmlDao = new XMLDAO();
        $sql = [];
        $data = $xmlDao->parseStruct($templatesFile, ['email']);
     ...
     * @return array|boolean
     */
    public function installEmailTemplateData($templateDataFile, $locale, $returnSql = false, $emailKey = null)
    {
        $xmlDao = new XMLDAO();
        $sql = [];
        $data = $xmlDao->parse($templateDataFile, ['email_texts', 'email_text', 'subject', 'body', 'description']);
        if (!$data) {
            return false;
        }
